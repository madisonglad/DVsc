...
class lf_opt:
    def __init__(self):
        ...
        parser.add_option("-w", "--whitelistfile", dest="whitelistfile", help="whitelistfile")
...
def filter_countbyumi(id,dict_virus_filter, umifilterfile, dict_umitotalreads):
    ...
    with open(id + '/countbyumi.txt') as f:
        for line in f:
            ...
            if virus_name in dict_virus_filter:
                umiid = line.split('\t')[1]
                num = line.split('\t')[2]
                rpm = float(num)*1000000/float(dict_umitotalreads[umiid])
                outf.write(line.strip() + '\t' + str(rpm) + '\n')
...
def main():
    ...
    whitelistfile = opt.options.whitelistfile
    umifilterfile = opt.options.output
    method = opt.options.method

    if method == 'scRNAseq':
        dict_umitotalreads = totalreads(whitelistfile)
        ...
        filter_countbyumi(sampleid, dict_virus_filter, umifilterfile, dict_umitotalreads)
